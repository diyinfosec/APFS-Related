...
struct omap_phys {
  ...
  omap_phys_t xid_t om_pending_revert_max;
};
struct omap_key {
  oid_t ok_oid;
  xid_t ok_xid;
...
struct apfs_superblock {
  ...
  apfs_superblock_t uint64_t apfs_total_blocks_alloced;
...
struct j_drec_val {
  ...
  uint64_t date_added;
  uint16_t flags;
  j_dir_stats_key_t uint8_t xfields[];
}
__attribute__((packed));
...
j_inode_flags;
typedef enum {
  XATTR_DATA_STREAM = 0x00000001, XATTR_DATA_EMBEDDED = 0x00000002, dir_rec_flags XATTR_FILE_SYSTEM_OWNED = 0x00000004, XATTR_RESERVED_8 = 0x00000008,
}
j_xattr_flags;
typedef enum {
...
__attribute__((packed));
typedef enum {
  SNAP_META_PENDING_DATALESS = 0x00000001, SNAP_META_MERGE_IN_PROGRESS = 0x00000002, snap_meta_ext_obj_phys_t
}
snap_meta_flags;
...
struct spaceman_phys {
  ...
  sfq uint64_t sm_fs_reserve_alloc_count;
